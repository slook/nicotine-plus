...
class BasePlugin:
    ...
    def send_private(self, user, text, show_ui=True, switch_page=True):
        ...
        if show_ui:
            core.privatechat.show_user(user, switch_page=switch_page)

        return core.privatechat.send_message(user, text)
            ...
            try:
                for trigger, data in plugin.commands.items():
                    ...
                    if '"' in args and "" in usage:
                        ...
                        # Don't require the usage of quotation marks
                    ...
                    if room is not None:
                        return_value = getattr(plugin, callback_name)(args, room=room)

                    elif user is not None:
                        return_value = getattr(plugin, callback_name)(args, user=user)
                ...
                if not command_found:
                    for trigger, func in legacy_commands:
                        if trigger == command:
                            ...
                            return_value = command_found = True
                            break
