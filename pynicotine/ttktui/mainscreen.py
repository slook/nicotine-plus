...
class MainScreen(Screen):
    ...
    def __init__(self, application):
        ...
        self.download_status_button.setToolTip("Downloading (Speed / Active Users)")

        self.upload_status_button = ttk.TTkLabel(
            parent=self.status_bar, minWidth=16, maxWidth=16, alignment=ttk.TTkK.RIGHT_ALIGN,
            text=ttk.TTkString("▲ 0 Kb/s ⓪", ttk.TTkColor.fg("#AAAAAA")).setColorAt(0, ttk.TTkColor.CYAN)
        )  #        ⇬⇯⇮⇧⇑⇪↥↟↑⇡⇞ 🮧🭯🮦🭭🮚   🭭🮷🮵🮶🮸 🭯   🮦🭮🮵🮶🭬🮧
        self.upload_status_button.setToolTip("Uploading (Speed / Active Users)")
        ...
        self.user_status_button = ttk.TTkButton(
            ...
            text=ttk.TTkString(USER_STATUS_LABELS[UserStatus.OFFLINE].upper(), ttk.TTkColor.BOLD), addStyle={
                'default': {
                    'color': ttk.TTkColor.BOLD + ttk.TTkColor.WHITE + USER_STATUS_COLORS[UserStatus.OFFLINE].invertFgBg(),
                    'borderColor': ttk.TTkColor.BLINKING,
                },
                'disabled': {
                    'color': ttk.TTkColor.RST + ttk.TTkColor.bg("#888888"),  # + ttk.TTkColor.ITALIC,
                    'borderColor': ttk.TTkColor.fg("#888888") + ttk.TTkColor.BLINKING,
                },
                'hover': {
                    'color': ttk.TTkColor.BOLD + ttk.TTkColor.YELLOW + ttk.TTkColor.BG_RED,
                    'borderColor': ttk.TTkColor.fg("#FFFFCC") + ttk.TTkColor.BLINKING,
                },
                'checked': {
                    'color': ttk.TTkColor.BOLD + ttk.TTkColor.BLACK + USER_STATUS_COLORS[UserStatus.AWAY].invertFgBg(),
                    'borderColor': ttk.TTkColor.YELLOW,
                },
                'unchecked': {
                    'color': ttk.TTkColor.BOLD + ttk.TTkColor.BLACK + USER_STATUS_COLORS[UserStatus.ONLINE].invertFgBg(),
                    'borderColor': ttk.TTkColor.GREEN,
                },
                'clicked': {
                    'color': ttk.TTkColor.BOLD + ttk.TTkColor.fg("#FFFFDD"),
                    'borderColor': ttk.TTkColor.fg("#DDDDDD") + ttk.TTkColor.BOLD,
                },
                'focus': {
                    'color': ttk.TTkColor.BOLD + ttk.TTkColor.fg("#dddd88") + ttk.TTkColor.bg("#000044"),
                    'borderColor': ttk.TTkColor.fg("#ffff00") + ttk.TTkColor.BLINKING,
                },
            }
        )
        ...
        self.downloads = ttk.TTkList(name="downloads")  # Downloads(self)
        self.uploads = ttk.TTkList(name="uploads")  # Uploads(self)
        self.buddies = Buddies(self)
        self.privatechat = PrivateChats(self)
        self.userinfo = ttk.TTkTabWidget(border=False, closable=True, name="userinfo")  # UserInfos(self)
    ...
    def show_tab(self, tab_number, tab_widget, tab_name):
        ...
        if not self.tab_bar.getWidgetByName(tab_name): # is not None:
            tab_text = TAB_LABELS.get(tab_name)
            tab_index = self.tab_bar.insertTab(tab_number, tab_widget, f"  {tab_text}  ", data=tab_name)  # , width=(len(tab_text)+8))
            # self.tab_bar.widget(tab_index).setWidth(len(tab_name)+8)

        self.tab_bar.tabButton(self.tab_bar.indexOf(tab_widget)).show()
    ...
    def update_user_status(self, *_args):
        ...
        if username is not None:
            self.user_status_button.setToolTip(ttk.TTkString(f"{status_icon} {username}", USER_STATUS_COLORS.get(status)))
        else:
            self.user_status_button.setToolTip("")

        if self.user_status_button.text() != status_text:
            self.user_status_button.setText(ttk.TTkString(status_text))

        if not self.user_status_button.isEnabled():
            self.user_status_button.setEnabled(True)

        self.user_status_button.setCheckable(status != UserStatus.OFFLINE)
    ...
    def set_status_text(self, text):
        ...
        w = self.status_label_container.width()
