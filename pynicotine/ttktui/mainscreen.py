...
class MainScreen(Screen):
    ...
    def __init__(self, application):
        ...
        self.status_bar.user_status_button.clicked.connect(self.on_toggle_status)
    ...
    def update_user_status(self, *_args):
        ...
        if username is not None:
            self.status_bar.user_status_button.setCheckable(True)
            self.status_bar.user_status_button.setChecked(is_away)
            self.status_bar.user_status_button.setToolTip(status_icon + ttk.TTkString(username))

        elif self.status_bar.user_status_button.isCheckable():
            self.status_bar.user_status_button.setCheckable(False)
            self.status_bar.user_status_button.setToolTip("")

        if str(self.status_bar.user_status_button.text()) != status_text:
            self.status_bar.user_status_button.setText(ttk.TTkString(status_text, ttk.TTkColor.BOLD))

        self.status_bar.user_status_button.setEnabled(True)
    ...
    def set_connection_stats(self, total_conns=0, download_bandwidth=0, upload_bandwidth=0, **_kwargs):
        ...
        if self.status_bar.connections_label.text() == total_conns_text:
            return
        ...
        self.status_bar.connections_label.setText(
    ...
    def update_log(self, timestamp_format, msg, title, level):
        ...
        self.log_view.log_callback_nicotine(timestamp_format, msg, title, level)
    ...
    def destroy(self):
        ...
        def describe_signal(signal):
            for ref in gc.get_referrers(signal):
                if ref is ttk.pyTTkSignal._signals:
                    continue
                if isinstance(ref, (list, dict, tuple)):
                    continue
                # ref is likely the owner object — search its slots
                for cls in type(ref).__mro__:
                    for slot in getattr(cls, '__slots__', []):
                        try:
                            if getattr(ref, slot, None) is signal:
                                return f"{type(ref).__qualname__}.{slot} @ {ref}"
                        except Exception:
                            pass
                # fallback: __dict__
                for k, v in getattr(ref, '__dict__', {}).items():
                    if v is signal:
                        return f"{type(ref).__qualname__}.{k} @ {ref}"
            return f"unowned signal{signal._types}"

        # Print only the connected signals
        for signal in ttk.pyTTkSignal._signals:
            if signal._connected_slots:
                slots_str = [getattr(s, '__qualname__', repr(s)) for s in signal._connected_slots]
                print(f" ▰ {describe_signal(signal)} → {slots_str}")

        print(f"Signals: {len(ttk.pyTTkSignal._signals)}")

        ttk.pyTTkSignal.clearAll()

        # for signal in ttk.pyTTkSignal._signals:
        #     print(f"{signal}")
        #     for _type in signal._types:
        #         print(f"{_type}")
        #     for _slot in signal._connected_slots:
        #         print(f"{_slot}")

        # print(f"_signals: {len(ttk.pyTTkSignal._signals)}")
        # print(f"_connected_slots: {len(ttk.pyTTkSignal._connected_slots)}")
        # print(f"_connected_async_slots: {len(ttk.pyTTkSignal._connected_async_slots)}")
        """
...
class LogView(Console):
    ...
    def log_callback_nicotine(self, timestamp_format, msg, _title, level):
