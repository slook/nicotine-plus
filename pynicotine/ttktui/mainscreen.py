...
class LogView(Console):
    ...
    def __init__(self, screen, **kwargs):
        ...
        self.log_categories_menu.addSpacer()
        self.log_categories_menu.ttk = self.log_categories_menu.addMenu(_("[Debug] TTkLog"), data="ttk", checkable=True)
        self.log_categories_menu.ttk.toggled.connect(self.on_log_ttk)
        self.log_categories_menu.ttk.setChecked(True)
    ...
    @ttk.pyTTkSlot(bool)
    def on_log_ttk(self, is_enabled):
        if is_enabled:
            ttk.TTkLog.installMessageHandler(self.ttk_log_callback)
            self._console_area._console_view.scroll_bottom(force=True)
        else:
            ttk.TTkLog._messageHandler.clear()

    def ttk_log_callback(self, mode, context, message):
        logType = "NONE"
        if mode == ttk.TTkLog.InfoMsg:       logType = ttk.TTkString("INFO"    , ttk.TTkColor.fg("#00ff00"))
        elif mode == ttk.TTkLog.DebugMsg:    logType = ttk.TTkString("DEBUG"   , ttk.TTkColor.fg("#00ffff"))
        elif mode == ttk.TTkLog.ErrorMsg:    logType = ttk.TTkString("ERROR"   , ttk.TTkColor.fg("#ff0000"))
        elif mode == ttk.TTkLog.FatalMsg:    logType = ttk.TTkString("FATAL"   , ttk.TTkColor.fg("#ff0000"))
        elif mode == ttk.TTkLog.WarningMsg:  logType = ttk.TTkString("WARNING" , ttk.TTkColor.fg("#ff0000"))
        elif mode == ttk.TTkLog.CriticalMsg: logType = ttk.TTkString("CRITICAL", ttk.TTkColor.fg("#ff0000"))
        ctx = ttk.TTkString(
