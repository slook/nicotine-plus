...
class FastConfigure(Dialog):
    ...
    def __init__(self, application):
        ...
        self.buttons_box = ttk.TTkContainer(layout=ttk.TTkHBoxLayout())
        self.previous_button = ttk.TTkButton(parent=self.buttons_box, text=_("_Previous"), border=True, maxHeight=3)
        self.previous_button.clicked.connect(self.on_previous)
        self.nav_spacer = ttk.TTkSpacer(parent=self.buttons_box)
        self.next_button = ttk.TTkButton(parent=self.buttons_box, text=_("_Next"), border=True, maxHeight=3)
        self.next_button.clicked.connect(self.on_next)

        # Setup Assistant Wizard Pager Window Content Container
        self.content_box = ttk.TTkContainer(layout=ttk.TTkVBoxLayout())
        ...
        self.port_page = ttk.TTkFrame(parent=self.content_box, title="Network Port", name="port_page")
    ...
    def init_window(self, layout):

        if self.welcome_page.layout().count():
            return

        self.create_welcome_page()
        self.create_account_page()
        self.create_port_page()
        ...
        self.texts = {
            "welcome_page": {
                #self.welcome_intro_text: ttk.TTkString(_("Nicotine+"), ttk.TTkColor.BOLD + NICOTINE_ICON_COLOR).align(width=welcome_guide_labels_box.width(), alignment=ttk.TTkK.CENTER_ALIGN),
                self.welcome_program_text: "Terminal client for the Soulseek peer-to-peer network"
            },
            "account_page": {
                self.account_intro_text: _("To create a new Soulseek account, fill in your desired username and password. If you already have an account, fill in your existing login details."),
                self.account_prompt_text: _("If your desired username is already taken, you will be prompted to change it."),
                self.invalid_username_text: _("Log in to an existing Soulseek account or create a new one. Usernames are case-sensitive and unique."),
                self.invalid_password_text: ""
            },
            "port_page": {
                self.port_intro_text: _("To connect with other Soulseek peers, a listening port on your router has to be forwarded to your computer."),
                self.port_users_text: _("If your listening port is closed, you will only be able to connect to users whose listening ports are open."),
                self.port_guide_text: _("If necessary, choose a different listening port below. This can also be done later in the preferences."),
            },
            "share_page": {
                self.share_intro_text: _("Soulseek users will be able to download from your shares. Contribute to the Soulseek network by sharing your own files and by resharing what you downloaded from other users.")
            },
            "summary_page": {
                self.summary_guide_text: _("Soulseek is an unencrypted protocol not intended for secure communication."),
                self.summary_donate_text: _("Donating to Soulseek grants you privileges for a certain time period. If you have privileges, your downloads will be queued ahead of non-privileged users.")
            }
        }
    ...
    def create_welcome_page(self):
        ...
        welcome_labels_box = ttk.TTkContainer(
            parent=self.welcome_page, layout=ttk.TTkVBoxLayout(), paddingTop=2, paddingLeft=2, paddingRight=2
        )
        self.welcome_main_icon = ttk.TTkSpacer(parent=welcome_labels_box)
        #self.welcome_main_icon = ttk.TTkImage(parent=welcome_labels_box, rasteriser=ttk.TTkImage.HALFBLOCK)
        #self.welcome_main_icon.setData(
        self.welcome_intro_label = ttk.TTkLabel(
            parent=welcome_labels_box,
            text=ttk.TTkString(_("Nicotine+"), ttk.TTkColor.BOLD + NICOTINE_ICON_COLOR),
            alignment=ttk.TTkK.CENTER_ALIGN,
            maxHeight=1
        )
        welcome_labels_box.layout().addWidget(ttk.TTkSpacer())
        welcome_widgets_box = ttk.TTkContainer(
            parent=self.welcome_page, layout=ttk.TTkHBoxLayout(), paddingBottom=1, paddingLeft=4, paddingRight=4
        )
        welcome_widgets_box.layout().addWidget(ttk.TTkSpacer())
        #self.welcome_intro_text = ttk.TTkTextEdit(parent=welcome_labels_box, readOnly=True)
        self.welcome_program_text = ttk.TTkTextEdit(parent=welcome_widgets_box, readOnly=True, minWidth=54)
        welcome_widgets_box.layout().addWidget(ttk.TTkSpacer())

    def create_account_page(self):

        account_guide_labels_box = ttk.TTkContainer(
            parent=self.account_page, layout=ttk.TTkVBoxLayout(), paddingTop=2, paddingLeft=2, paddingRight=2
        )
        ...
        account_widgets_box = ttk.TTkContainer(
            parent=self.account_page, layout=ttk.TTkGridLayout(), paddingBottom=1, paddingLeft=4, paddingRight=4
        )
        ...
        self.username_entry.returnPressed.connect(self.on_account_entry_activate)
        ...
        self.password_entry.returnPressed.connect(self.on_account_entry_activate)
        ...
        self.account_errors_label = ttk.TTkLabel(parent=self.account_page, alignment=ttk.TTkK.CENTER_ALIGN, maxHeight=1)

    def create_port_page(self):

        port_labels_box = ttk.TTkContainer(
            parent=self.port_page, layout=ttk.TTkVBoxLayout(), paddingTop=1, paddingLeft=2, paddingRight=2
        )
        ...
        self.port_errors_label = ttk.TTkLabel(parent=self.port_page, alignment=ttk.TTkK.CENTER_ALIGN, maxHeight=1)

    def create_share_page(self):

        download_frame = ttk.TTkFrame(
            parent=self.share_page, title=_("Download Files to Folder"), layout=ttk.TTkGridLayout(rowMinHeight=1)
        )
        download_frame.layout().addWidget(ttk.TTkSpacer(maxWidth=8), 1, 0)
        download_frame.layout().addWidget(ttk.TTkSpacer(maxWidth=8), 1, 3)
        ...
        self.download_errors_label = ttk.TTkLabel(parent=download_frame, alignment=ttk.TTkK.CENTER_ALIGN, maxHeight=1)

        download_frame.layout().addWidget(self.download_folder_button, 1, 1)
        download_frame.layout().addWidget(self.default_download_folder_button, 1, 2)
        download_frame.layout().addWidget(self.download_errors_label, 2, 0, 1, 3)

        share_frame = ttk.TTkFrame(parent=self.share_page, title=_("Share Folders"), layout=ttk.TTkVBoxLayout())
        share_widgets_box = ttk.TTkContainer(
            parent=share_frame, layout=ttk.TTkVBoxLayout(),
        ...
        self.shares_list_view.setColumnWidth(1, 36)
        ...
        shares_buttons_box = ttk.TTkContainer(
            parent=share_widgets_box, layout=ttk.TTkHBoxLayout(), paddingLeft=1, paddingRight=1
        )
        self._add_shared_folder_button = ttk.TTkFileButtonPicker(
            parent=shares_buttons_box, text=_("Add…"), maxheight=1, caption=_("Select a Folder") + " to Share",
        ...
        self._edit_shared_folder_button = ttk.TTkButton(parent=shares_buttons_box, text=_("Edit…"), maxheight=1, enabled=False)
        self._edit_shared_folder_button.clicked.connect(self.on_edit_shared_folder)

        self._remove_shared_folder_button = ttk.TTkButton(parent=shares_buttons_box, text=_("Remove"), maxheight=1, enabled=False)
        ...
        self.share_count_label = ttk.TTkLabel(
            parent=shares_buttons_box, text="Listing shares…", alignment=ttk.TTkK.RIGHT_ALIGN, maxHeight=1
        )
        self.shares_list_view._treeView.viewSizeChanged.connect(self.on_shares_list_changed)

        self.share_status_label = ttk.TTkLabel(parent=share_frame, alignment=ttk.TTkK.CENTER_ALIGN, maxHeight=1)

    def create_summary_page(self):

        summary_guide_labels_box = ttk.TTkContainer(
            parent=self.summary_page, layout=ttk.TTkVBoxLayout(), paddingTop=2, paddingLeft=2, paddingRight=2
        )
        self.summary_done_icon = ttk.TTkSpacer(parent=summary_guide_labels_box)  ##
        self.summary_intro_label = ttk.TTkLabel(
            parent=summary_guide_labels_box,
            text=ttk.TTkString(_("You are ready to use Nicotine+!"), ttk.TTkColor.BOLD),
            alignment=ttk.TTkK.CENTER_ALIGN,
            maxHeight=1
        )
    ...
    def reset_completeness(self):
        ...
        page_complete = (
            ...
            or (page == self.share_page and self.download_folder_button.path())
        )
    ...
    def _validate_username_entry(self):
        ...
        self.account_errors_label.setText(error)
        self.account_errors_label.setColor(ttk.TTkColor.BOLD+ttk.TTkColor.BG_RED if error else ttk.TTkColor.RST)
    ...
    def _validate_password_entry(self):
        ...
        if self.username_entry.validated and self.username_entry.text() and not entry:
    ...
    def _validate_port_entry(self):
        ...
        self.port_errors_label.setText(error)
        self.port_errors_label.setColor(ttk.TTkColor.BOLD+ttk.TTkColor.BG_RED if error else ttk.TTkColor.RST)
    ...
    def on_account_entry_activate(self):

        if not self._validate_username_entry():
            self.account_errors_label.setColor(self.account_errors_label.color()+ttk.TTkColor.BLINKING)
    ...
    def on_download_folder_selected(self, path):
        ...
        try:
            ...
            self.port_errors_label.setColor(ttk.TTkColor.RST)

        except OSError as error:
            self.port_errors_label.setColor(ttk.TTkColor.BOLD+ttk.TTkColor.BG_RED)

        self.download_errors_label.setText(error)
    ...
    def on_shared_folder_clicked(self, share_item, _col):
        is_selections = share_item.isSelected() or bool(self.shares_list_view.selectedItems())
    ...
    def on_shares_list_changed(self):
        self.share_count_label.setText("%i public shares" % self.shares_list_view.invisibleRootItem().size())
    ...
    def on_add_shared_folder_selected(self, folder_path):
        ...
        if (virtual_name, folder_path) in core.shares.check_shares_available():
    ...
    def on_remove_shared_folder(self):
        ...
        for share_item in reversed(self.shares_list_view.selectedItems()):
            ...
            if core.shares.remove_share(virtual_name):
                self.share_status_label.setText(
                    ttk.TTkString(_("Removed share \"%s\" (rescan required)") % virtual_name, ttk.TTkColor.GREEN))
            else:
                self.share_status_label.setText(
                    ttk.TTkString(_("No share with name \"%s\"") % virtual_name, ttk.TTkColor.BG_RED))

            share_item.setSelected(False)  ##
    ...
    def on_page_change(self, page):
        ...
        self.previous_button.setText(_("_Cancel") if self.invalid_password else _("_Previous"))
        ...
        if page == self.account_page:
            ...
            self.listen_port_entry.setFocus()  ## FIXME: widget invisible
            self.next_button.setFocus()
        # elif page == self.share_page:
        #    pass  # self.download_folder_button.setFocus()
        else:
            self.next_button.setFocus()
    ...
    def on_next(self):
        ...
        if self.finished or next_index >= len(self.pages):
            self.on_finished(self.rescan_required, self.invalid_password)
            return
    ...
    def on_finished(self, rescan_required, account_required):
        ...
        # account_page
        if account_required or config.need_config():
            config.sections["server"]["login"] = str(self.username_entry.text())
            config.sections["server"]["passw"] = str(self.password_entry.text())
    ...
    def on_close(self, window):
        ...
        self.account_errors_label.setText("")
        self.share_status_label.setText("")
    ...
    def on_show(self, window):
        ...
        self.init_window(window.layout())

        self.account_page.setEnabled(self.invalid_password or config.need_config())
        self.set_visible_page(self.account_page if self.invalid_password else self.welcome_page)
        ...
        if self.invalid_password:
            self.invalid_password_text.setText(ttk.TTkString(
                _("User %s already exists, and the password you entered is invalid. Please choose another username "
                  "if this is your first time logging in.") % config.sections["server"]["login"], ttk.TTkColor.YELLOW))

        self.invalid_username_text.setVisible(self.invalid_password)  # TODO: INVALIDUSER
        self.invalid_password_text.setVisible(self.invalid_password)
        self.account_intro_text.setVisible(not self.invalid_password)
        self.account_prompt_text.setVisible(not self.invalid_password)

        # port_page
        self.on_default_port(factory=False)
        #listen_port, _unused_port = config.sections["server"]["portrange"]
        #self.listen_port_entry.setValue(listen_port)
        ...
        unavailable_shares = core.shares.check_shares_available()

        for virtual_name, folder_path, *_unused in config.sections["transfers"]["shared"]:
            if (virtual_name, folder_path) in unavailable_shares:
                icon = ttk.TTkString(" 🛇 ", ttk.TTkColor.BOLD + ttk.TTkColor.RED)
                icon_label = _("Unreadable")
            else:
                icon = ""
