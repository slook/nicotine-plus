...
class FastConfigure(Dialog):
    ...
    def __init__(self, application):
        ...
        self.isolated_mode = application.isolated_mode
        ...
        self.welcome_page = ttk.TTkFrame(
            parent=self.content_box, title=_("Welcome to Nicotine+"), name="welcome_page", visible=False
        )
        self.account_page = ttk.TTkFrame(
            parent=self.content_box, title="Soulseek Account", name="account_page", visible=False
        )
        self.port_page = ttk.TTkFrame(
            parent=self.content_box, title="Network Port", name="port_page", visible=False
        )
        self.share_page = ttk.TTkContainer(
            parent=self.content_box, name="share_page", visible=False
        )
        self.summary_page = ttk.TTkFrame(
            parent=self.content_box, title="Setup Finished", name="summary_page", visible=False
        )
        self.spacer_page = ttk.TTkFrame(
            parent=self.content_box, title="", border=False, name="spacer_page"  # placeholder to set initial geometry
        )

        self.pages = [
            self.welcome_page, self.account_page, self.port_page, self.share_page, self.summary_page, self.spacer_page
        ]
    ...
    def load(self, parent):
        ...
        self.create_spacer_page(self.spacer_page)  # set size first to avoid initial shuffling around of widgets
        ...
        self.texts = {
            ...
            "port_page": {
                ...
                self.port_guide_text: _(
                    ...
                    "preferences."
                ) if core.cli_listen_port is None else _(
                    "The following listening port has been set by your environment:"
                )
            },
            "share_page": {
    ...
    def create_welcome_page(self, parent):
        ...
        welcome_widgets_box.layout().addWidget(ttk.TTkSpacer(maxWidth=6, maxHeight=2))
        self.welcome_intro_text = ttk.TTkTextEditView(parent=welcome_widgets_box, readOnly=True)
        welcome_widgets_box.layout().addWidget(ttk.TTkSpacer(maxWidth=2, maxHeight=2))

        parent.layout().addWidget(ttk.TTkSpacer(maxHeight=3))
    ...
    def create_account_page(self, parent):
        ...
        self.account_intro_text = ttk.TTkTextEditView(parent=account_guide_labels_box, readOnly=True)
        self.account_prompt_text = ttk.TTkTextEditView(parent=account_guide_labels_box, readOnly=True)
        self.invalid_username_text = ttk.TTkTextEditView(parent=account_guide_labels_box, readOnly=True, visible=False)
        self.invalid_password_text = ttk.TTkTextEditView(parent=account_guide_labels_box, readOnly=True, visible=False)
    ...
    def create_port_page(self, parent):
        ...
        self.port_intro_text = ttk.TTkTextEditView(parent=port_labels_box, readOnly=True)
        self.port_users_text = ttk.TTkTextEditView(parent=port_labels_box, readOnly=True)
        self.port_guide_text = ttk.TTkTextEditView(parent=port_labels_box, readOnly=True)
    ...
    def create_share_page(self, parent):
        ...
        self.download_frame = ttk.TTkFrame(
            parent=parent, title=_("Finished downloads:"), layout=ttk.TTkGridLayout(rowMinHeight=1)
        )
        self.download_frame.layout().addWidget(ttk.TTkSpacer(maxWidth=8), 1, 0)
        self.download_frame.layout().addWidget(ttk.TTkSpacer(maxWidth=8), 1, 2)
        ...
        self.download_folder_button = FileChooserButton(
            callback=self.on_download_folder_selected, path=config.defaults["transfers"]["downloaddir"],
            fileMode=ttk.TTkK.FileMode.Directory, show_open_external_button=not self.isolated_mode
        )
        self.download_info_bar = ttk.TTkLabel(alignment=ttk.TTkK.CENTER_ALIGN, maxHeight=1)

        self.download_frame.layout().addWidget(self.download_folder_button.parentWidget(), 1, 1)  # Box()
        self.download_frame.layout().addWidget(self.download_info_bar, 2, 0, 1, 3)
        ...
        self.share_intro_text = ttk.TTkTextEditView(parent=share_widgets_box, readOnly=True, maxHeight=4)
    ...
    def create_summary_page(self, parent):
        ...
        summary_guide_labels_box.layout().addWidget(ttk.TTkSpacer())
        self.summary_guide_text = ttk.TTkTextEditView(parent=summary_guide_labels_box, readOnly=True)
        self.summary_donate_text = ttk.TTkTextEditView(parent=summary_guide_labels_box, readOnly=True)
        summary_guide_labels_box.layout().addWidget(ttk.TTkSpacer())

    # def create_finish_page(self, parent):

    def create_spacer_page(self, parent):

        if parent.layout().count():
            return

        finish_guide_labels_box = ttk.TTkContainer(
            parent=parent, layout=ttk.TTkVBoxLayout(), paddingTop=2, paddingLeft=2, paddingRight=2
        )
        finish_guide_labels_box.layout().addWidget(ttk.TTkSpacer())
    ...
    def _validate_password_entry(self):
        ...
        is_username_validated = (self.username_entry.validated and self.username_entry.text())

        if not entry and is_username_validated and self.password_entry.hasFocus():
    ...
    def on_default_port(self, factory=True):
        ...
        if core.cli_listen_port is not None:
            self.listen_port_entry.setValue(core.cli_listen_port)
            self.listen_port_entry.setEnabled(False)
            self.default_port_button.setEnabled(False)

        else:
            if self.listen_port_entry.value() != config.sections["server"]["portrange"][0]:
                factory = False  # Undo

            port, _un = config.defaults["server"]["portrange"] if factory else config.sections["server"]["portrange"]
    ...
    def _validate_download_folder_path(self):
        ...
        error = ""
        try:
            self.download_folder_button.default_path_button.setEnabled(
                not os.path.samefile(
                    os.path.expandvars(self.download_folder_button.path()),
            ...
            if not config.need_config():
                # Default folder doesn't exist until first download
                error = err
    ...
    def on_page_change(self, page):
        ...
        self.previous_button.clearFocus()
        self.next_button.setFocus()
    ...
    def on_show(self, window):
        ...
        # share_page
        self.on_download_folder_selected(core.downloads.get_default_download_folder())
