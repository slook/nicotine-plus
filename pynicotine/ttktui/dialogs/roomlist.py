...
class RoomList(ttk.TTkTree):
    ...
    def create_dialog(self, content_box):
        ...
        top_bar = ttk.TTkContainer(layout=ttk.TTkHBoxLayout(), maxHeight=1)
        ...
        content_box.layout().addWidget(top_bar)

        self.list_container = ttk.TTkFrame(layout=ttk.TTkGridLayout())
        self.list_container.layout().addWidget(self)
        # self.list_container.setParent(self)
        content_box.layout().addWidget(self.list_container)

        foot_bar = ttk.TTkContainer(layout=ttk.TTkVBoxLayout(), paddingLeft=1, maxHeight=2)
        foot_bar.layout().addWidget(self.public_feed_toggle)
        foot_bar.layout().addWidget(self.room_invitations_toggle)
        content_box.layout().addWidget(foot_bar)
    ...
    def present(self):
        ...
        if self.content_box is None:
            ...
            self.create_dialog(self.content_box)
        ...
        self.window = Dialog(
            parent=self.application._instance,  # self.screen,
            content_box=self.content_box,
            default_widget=self.search_entry,
            show_callback=self.on_dialog_show,
            close_callback=self.on_dialog_close,
            title=_("All Rooms") if self.room_items else _("No Chat Rooms"),
            width=min(49, self.screen.width()),
            height=int(0.8 * self.screen.height())
        )
        self.window.present()
        # self.window = self.dialog.window

    def on_dialog_show(self, _window):

        self.refresh_button.setEnabled(True)
        self.itemActivated.connect(self.on_room_item_activated)
        self._treeView.focusChanged.connect(self.on_focus_changed)
    ...
    def add_room(self, room, user_count=0, is_private=False, is_owner=False, is_joined=None):
        ...
        if not self.search_entry.isEnabled():
            if self.window is not None:
                self.window.window.setTitle(_("All Rooms"))
    ...
    @ttk.pyTTkSlot(bool)
    def on_focus_changed(self, is_focused):
        if not is_focused and self.popup_menu is None:
            self.clearSelection()
    ...
    def on_search(self, text):
        ...
        if any(room_item.isHidden() for room_item in self.room_items.values()):
            self.window.window.setTitle(_("Search rooms…"))
        else:
            self.window.window.setTitle(_("All Rooms"))

        if all(room_item.isHidden() for room_item in self.room_items.values()):
