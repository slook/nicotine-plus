...
class SharesPage(ttk.TTkFrame):
    ...
    def create_page(self, frame):
        ...
        for col, width in enumerate([3, 32, 25, 16]):
        ...
        for col, width in enumerate([3, 60, 15]):
    ...
    @ttk.pyTTkSlot(ttk.TTkTreeWidgetItem, int)
    def on_filter_clicked(self, filter_item, _col):
        is_selections = filter_item.isSelected() or bool(self.filter_list_view.selectedItems())
...
class ChatsPage(ttk.TTkFrame):
    ...
    def create_page(self, frame):
        ...
        self.private_room_toggle = ttk.TTkCheckbox(parent=invite_box, text=_("Accept private room invitations"))
        ...
        format_box = ttk.TTkContainer(parent=times_container, layout=ttk.TTkHBoxLayout(), padding=(0, 0, 0, 1))
        self.format_codes_label = ttk.TTkLabel(
            ...
            text=ttk.TTkString(
                ("Format codes"),
                ttk.TTkColor.fg(
                    config.sections["ui"].get("urlcolor", "").upper() or "#5288CE",
                    link="https://docs.python.org/3/library/datetime.html#format-codes"
                )
            )
        )
    ...
    def get_settings(self):
        ...
        return {
            "server": {
                ...
                "private_chatrooms": self.private_room_toggle.isChecked()
            },
            "logging": {
...
class PluginsPage(ttk.TTkFrame):
    ...
    class PluginItem(ttk.TTkTreeWidgetItem):
        ...
        def __init__(self, data, selected=False, **kwargs):
    ...
    def create_page(self, frame):
        ...
        frame.setLayout(layout=ttk.TTkHBoxLayout())

        plugin_list_container = ttk.TTkFrame(layout=ttk.TTkVBoxLayout(), title=_("Plugins"))

        self.enable_plugins_toggle = ttk.TTkCheckbox(parent=plugin_list_container, text=_("Enable plugins"))
        self.enable_plugins_toggle.toggled.connect(self.on_enable_plugins_toggled)

        plugin_list_container.layout().addWidget(ttk.TTkSpacer(maxHeight=1))

        self.plugin_list_view = ttk.TTkTree(
            parent=plugin_list_container,
            header=[_("Enabled"), _("Plugin")],
            minHeight=3
        )
        self.plugin_list_view.setColumnWidth(0, 8)
        self.plugin_list_view.setColumnWidth(1, 32)
        ...
        buttons_box = ttk.TTkContainer(
            parent=plugin_list_container, layout=ttk.TTkHBoxLayout(), maxHeight=1, paddingLeft=1, paddingRight=1
        )
        ...
        frame.layout().addWidget(plugin_list_container)
    ...
    def set_settings(self):
        ...
        for plugin_name in core.pluginhandler.list_installed_plugins():
            ...
            plugin_item = self.PluginItem([enabled_checkbox, plugin_human_name], selected=False)
...
class Preferences(Dialog):
    ...
    def __init__(self, application):
        ...
        self.content_box.setSizes([24, 93])
    ...
    def on_close(self, _window):
        pass  ## self.content.get_vadjustment().set_value(0)
