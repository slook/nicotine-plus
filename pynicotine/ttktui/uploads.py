...
class Uploads(Transfers):
    ...
    def start(self):
        ...
        self.tree = UploadsTree(parent=self.tree_container, name=self.name())

        upload_status_text = f"{0:5d} KiB/s 🡱"
        self.upload_status_label = ttk.TTkLabel(
            minWidth=16, maxWidth=16, alignment=ttk.TTkK.RIGHT_ALIGN,
            text=(
        ...
        events.schedule(delay=0.5, callback=self.init_uploads)

    def init_uploads(self):
        self.screen.status_bar.uploads_status_container.layout().addWidget(self.upload_status_label)
    ...
    def set_connection_stats(self, upload_bandwidth=0, **_kwargs):
        ...
        if self.upload_status_label is None or self.upload_status_label.text() == upload_status_text:
            return
