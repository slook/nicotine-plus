...
class Application(TTk):
    ...
    def __init__(self, ci_mode):
        ...
        self.ci_mode = ci_mode  # Continuous Integration tests in progress
        self.screen = None      # TTk Root Container will run in its own thread

        # Show errors on the NORMAL_SCREEN
    ...
    def create_file_menu(self, header_menu, _position=None):
        ...
        self.file_menu._spacer1     = self.file_menu.addSpacer()
        self.file_menu._preferences = self.file_menu.addMenu(_("_Preferences"))
        self.file_menu._preferences.menuButtonClicked.connect(self.on_preferences)
        self.file_menu._spacer2     = self.file_menu.addSpacer()
        self.file_menu._quit        = self.file_menu.addMenu(("_Quit"))
        self.file_menu._quit.menuButtonClicked.connect(self.on_quit_request)

    def create_shares_menu(self, header_menu, _position=None):

        self.shares_menu = header_menu.addMenu(_("_Shares").replace("_", "&"))
        self.shares_menu._rescan = self.shares_menu.addMenu(_("_Rescan Shares")).menuButtonClicked.connect(self.on_rescan_shares)
        self.shares_menu._config = self.shares_menu.addMenu(_("Configure _Shares")).menuButtonClicked.connect(self.on_configure_shares)
        self.shares_menu._spacer = self.shares_menu.addSpacer()
        self.shares_menu._publics = self.shares_menu.addMenu(_("Browse _Public Shares")).menuButtonClicked.connect(self.on_browse_public_shares)
        self.shares_menu._buddies = self.shares_menu.addMenu(_("Browse _Buddy Shares")).menuButtonClicked.connect(self.on_browse_buddy_shares)
        self.shares_menu._trusteds = self.shares_menu.addMenu(_("Browse _Trusted Shares")).menuButtonClicked.connect(self.on_browse_trusted_shares)
    ...
    def create_help_menu(self, header_menu, position=None):
        ...
        self.help_menu._keyboards = self.help_menu.addMenu(_("_Keyboard Shortcuts")).menuButtonClicked.connect(self.on_keyboard_shortcuts)
        self.help_menu._setup_ass = self.help_menu.addMenu(_("_Setup Assistant")).menuButtonClicked.connect(self.on_fast_configure)
        self.help_menu._statistics = self.help_menu.addMenu(_("_Transfer Statistics")).menuButtonClicked.connect(self.on_transfer_statistics)
        self.help_menu._spacer1 = self.help_menu.addSpacer()
        self.help_menu._bug = self.help_menu.addMenu(_("Report a _Bug")).menuButtonClicked.connect(self.on_report_bug)
        self.help_menu._translate = self.help_menu.addMenu(_("Improve T_ranslations")).menuButtonClicked.connect(self.on_improve_translations)
        self.help_menu._spacer2 = self.help_menu.addSpacer()
        self.help_menu._about = self.help_menu.addMenu(_("_About Nicotine+").replace("_", "&")).menuButtonClicked.connect(self.on_about)
    ...
    def create_log_menu(self, footer_menu, position=None):
        ...
        self.log_menu = footer_menu.addMenu(" ⁝ ", alignment=position)
        self.log_menu._catagories = self.log_menu.addMenu(_("Catagories"))
        self.log_menu._catagories2 = self.log_menu.addMenu(_("Catagories 2"))
        self.log_menu._spacer = self.log_menu.addSpacer()
        self.log_menu._show = self.log_menu.addMenu(
            _("Show Log Pane"), checkable=True, checked=(not config.sections["logging"]["logcollapsed"])
        )
        self.log_menu._show.toggled.connect(self.screen.on_show_log_pane)
        self.log_menu.setToolTip(_("Log Pane"))
    ...
    def _update_user_status(self, *_args):
        ...
        for action in (self.file_menu._disconnect, self.file_menu._privileges):  #, "away-accel",
