...
class Application:
    ...
    def run(self):
        ...
        # core.enabled_components.discard("cli")  # Don't start the core input processor
        core.init_components(disabled_components={"signal_handler", "cli"}, isolated_mode=self.isolated_mode)
        core.start()
    ...
    def _create_file_menu(self, menu_button):
        ...
        menu_button.menuButtonClicked.connect(self._update_user_status)
    ...
    def on_shares_unavailable(self, shares):
        ...
        events.schedule(delay=0.2, callback=dialog.present)
    ...
    def on_preferences(self, *_args, page_name=None):
        ...
        if page_name:
            ...
            }.get(self.screen.tab_bar.currentWidget().name(), "network")
            self.preferences.set_active_page(page_name)
