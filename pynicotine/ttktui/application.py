...
class Application(TTk):
    ...
    def on_fast_configure(self, *_args, invalid_password=False):
        ...
        if invalid_password or self.fast_configure.invalid_password:
            self.fast_configure.invalid_password = invalid_password

            # Workaround dialog made inviisble by other widgets updating
            events.schedule(delay=2, callback=self.fast_configure.present)
            return
