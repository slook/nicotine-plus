...
class ChatRooms(Pages):
    ...
    def __init__(self, screen, name="chatrooms"):
        ...
        _gap = ttk.TTkSpacer(parent=self.header, minWidth=1, maxWidth=1)
        ...
        _expander_top = ttk.TTkSpacer(parent=self._spacer)
        _expander_mid = ttk.TTkSpacer(parent=self._spacer)
        _placeholder = ttk.TTkLabel(
            parent=self._spacer, enabled=False, alignment=ttk.TTkK.CENTER_ALIGN,
            text=_("Join a room to chat with other online users, or create a private room with restricted access")
        )
        _expander_bottom = ttk.TTkSpacer(parent=self._spacer)
...
class ChatRoom(Chatter):
    ...
    def add_user_row(self, userdata):
        ...
        self.user_items[username] = user_item = UserItem([
            ...
            h_speed,
            h_files,
        ], speed_data=speed, files_data=files)
    ...
    def user_stats(self, msg):
        ...
        user_item.speed_data = msg.avgspeed or 0
        user_item.files_data = msg.files or 0

        h_speed = human_speed(user_item.speed_data) if user_item.speed_data > 0 else ""
        h_files = humanize(user_item.files_data)
...
class UserItem(ttk.TTkTreeWidgetItem):
    ...
    def __init__(self, data, speed_data=0, files_data=0, **kwargs):
    ...
    def sortData(self, col):
        ...
        return str(self.data(col)).lower()
