...
class ChatRooms(Pages):
    ...
    def create_header(self):
        ...
        self.chatrooms_entry = ttk.TTkLineEdit(parent=self.header_bar, hint=_("Join or create room…"))
        ...
        self.chatrooms_button.clicked.connect(self.screen.application.on_room_list)
    ...
    @ttk.pyTTkSlot(ttk.TTkWidget, ttk.TTkMouseEvent)  # not implemented rightClick contextClick alternateClick
    def on_page_menu(self, page_button, evt):

        if self.popup_menu is not None:
            self.popup_menu.remove_page.menuButtonClicked.clear()
            self.popup_menu.remove_page.close()
        ...
        self.popup_menu.remove_page = self.popup_menu.addMenu(_("_Leave Room"))
        self.popup_menu.remove_page.menuButtonClicked.connect(page.on_close)
...
class ChatRoom(Chatter):
    ...
    def __init__(self, chats, room, is_private, is_global):
        ...
        self.users_list = None  # RoomUsersList(parent=self.users_container)  # , room=room)
        self.user_items = {}
    ...
    def add_user_row(self, userdata):  # , username_color=None):
        ...
        if self.users_list is None:
            ...
            self.user_items = self.users_list.iterators
            self.users_spacer.setVisible(False)

        username = userdata.username

        old_user_item = self.user_items.get(username)
    ...
    def populate_room_users(self, joined_users):
        ...
        self.users_list.add_rows(
            ...
            for user_data in joined_users if user_data.username not in self.user_items
        )
        ...
        if private_room is not None:
            ...
            self.users_list.add_rows(
                ...
                for username in private_room.members if username not in self.user_items
            )

            if owner and owner not in self.user_items:
                self.users_list.add_row(UserData(owner, status=UserStatus.OFFLINE))

        self.update_user_count()
        self.users_list.resizeColumnToContents(3)  # Co
        # self.users_list._treeView.setSortingEnabled(True)
        self.users_list.sortItems(self.users_list._SORT_COLUMN, self.users_list._SORT_ORDER)
    ...
    def update_user_count(self):
        self.users_header.setTitle(
            f'{len(self.user_items)} {_("Members") if self.is_private else _("Users")}')
    ...
    def on_toggle_user_list_visibility(self, visible):
        ...
        self.user_list_button.setToolTip(_("Hide Room Users") if visible else _("Show Room Users"))
        ...
        if self.chats.screen.tab_bar is None:
            # Initializing screen at startup
            return
...
class RoomUsersList(UsersList):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        if parent is None:
            # Global room doesn't have any items in its user list iterators
            parent = ttk.TTkContainer(layout=ttk.TTkVBoxLayout(), name="global_feed", visible=False)
            # return
    ...
    def remove_user_item(self, user_item):
        ...
        if user_item != old_user_item:
            raise KeyError(f"{user_item} is not {old_user_item}")  # Debug
