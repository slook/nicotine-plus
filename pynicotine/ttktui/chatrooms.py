...
class ChatRooms(Pages):
    ...
    def show_room(self, room, is_private=False, switch_page=True, remembered=False):
        ...
        if room not in self.pages:
            ...
            tab_position = 0 if is_global and not remembered else self.count()

            self.pages[room] = page = ChatRoom(self, room, is_private=is_private, is_global=is_global)
            tab_index = self.insertTab(tab_position, page, f" {room} ", data=room)
            self.tabButton(tab_index).closeClicked.connect(page.on_close)

            self.update_page_tab_button(room)
            self.update_page_menu_item(room)
...
class ChatRoom(Chatter):
    ...
    def __init__(self, chats, room, is_private, is_global):
        ...
        self.users_list = ttk.TTkTree(parent=self.users_container, name=room)
        # self.users_list.setFocusPolicy(ttk.TTkK.FocusPolicy.StrongFocus)
        self.users_list.setHeaderLabels([
            " ",
            _("User"),
            _("Country"),
            _("Speed"),
        ...
        for col, width in enumerate([3, 30, 7, 12, 8]):
            self.users_list.setColumnWidth(col, width)
...
class UserItem(ttk.TTkTreeWidgetItem):
    ...
    def __init__(self, data, select_row=False, **kwargs):
        ...
        #self.setIcon(0, USER_STATUS_ICONS[UserStatus.OFFLINE])
        #self.user = data[1]
