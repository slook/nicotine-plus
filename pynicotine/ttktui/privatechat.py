...
class PrivateChats(Pages):
    ...
    def __init__(self, screen, name="private"):
        ...
        self.username_entry = ttk.TTkLineEdit(parent=self.header, hint=_("Username…"))  # , alignment=ttk.TTkK.CENTER_ALIGN)  #, minWidth=30)
        self.username_entry.setMinimumWidth(30)
        self.username_entry.setMaximumWidth(30)
        ...
        self.private_history_button = ttk.TTkButton(parent=self.header, text=ttk.TTkString("Chat History"))
        self.private_history_button.setFocusPolicy(ttk.TTkK.FocusPolicy.StrongFocus)
        ...
        tooltip = _("Enter the name of a user to start a text conversation with them in private")
    ...
    def on_page_menu(self, page_button, evt):
        ...
        self.popup_menu = UserPopupMenu(page_button.parentWidget(), page.name(), "privatechat")
    ...
    def on_get_page(self, caller):
        ...
        if isinstance(caller, ttk.TTkMenuButton):
            ...
            username = str(caller.text().toAscii()).strip()
            caller.setText("")
