...
class Downloads(Transfers):
    ...
    def start(self):
        ...
        self.tree = DownloadsTree(parent=self.tree_container, name=self.name())

        download_status_text = f"{0:5d} KiB/s 🡳"
        self.download_status_label = ttk.TTkLabel(
            minWidth=16, maxWidth=17, alignment=ttk.TTkK.RIGHT_ALIGN,
            text=(
        ...
        events.schedule(delay=0.7, callback=self.init_downloads)

    def init_downloads(self):
        self.screen.status_bar.downloads_status_container.layout().addWidget(self.download_status_label)
    ...
    def set_connection_stats(self, download_bandwidth=0, **_kwargs):
        ...
        if self.download_status_label is None or self.download_status_label.text() == download_status_text:
            return
