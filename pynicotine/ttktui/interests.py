...
class Interests(ttk.TTkSplitter):
    ...
    def create_header(self):
        ...
        self.interests_title = ttk.TTkLabel(
            ...
            text=ttk.TTkString(self.screen.TAB_LABELS[self.name()], ttk.TTkColor.BOLD)
        )
    ...
    def on_view_resize(self, w, h):
        ...
        new_orientation = ttk.TTkK.Direction.HORIZONTAL if w > (h * 2) else ttk.TTkK.Direction.VERTICAL
    ...
    def add_thing_i_like(self, item, select_row=True):
        ...
        if self.interests_picker.likes_list.indexOfTopLevelItem(item) < 0:
            self.interests_picker.likes_list.addTopLevelItem(item)

        if select_row:
            self.interests_picker.likes_list.selectItem(item)
    ...
    def add_thing_i_hate(self, item, select_row=True):
        ...
        if self.interests_picker.hates_list.indexOfTopLevelItem(item) < 0:
            self.interests_picker.hates_list.addTopLevelItem(item)

        if select_row:
            self.interests_picker.hates_list.selectItem(item)
...
class InterestsPicker(ttk.TTkSplitter):
    ...
    def on_resize(self, w, h):
        ...
        if self.orientation() != new_orientation:  # and self.orientation_button.data() == "auto"
...
class _InterestsList(ttk.TTkTree):
    ...
    def clear(self):
        """Removes all items and disconnects the signals"""
    ...
    def create_popup_menu(self, item):
        ...
        self.popup_menu = PopupMenu(self, closed_callback=self.on_popup_closed, title=recommendation)
...
class SimilarUsersViewer(ttk.TTkFrame):
    ...
    def set_similar_users(self, users, item=None):
        ...
        self.similar_users_list.sortItems(
            self.similar_users_list.sortColumn(), self.similar_users_list._treeView._sortOrder
        )
