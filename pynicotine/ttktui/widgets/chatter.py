...
class Chatter(ttk.TTkSplitter):
    ...
    def __init__(self, chats, entity, send_message_callback, command_callback, follow=True):
        ...
        self.fal = 0  # Full Area Length total number of wrapped lines
        self.follow = follow
    ...
    def load(self, is_enabled=False):
        ...
        self.chat_container.sizeChanged.connect(self.on_view_size_changed)  # rewrap
    ...
    def destroy(self):
        ...
        #self.chat_view.viewport().viewSizeChanged.disconnect(self.on_view_size_changed)
    ...
    def on_view_size_changed(self, w, h):
        self.scroll_bottom(snap=h)
    ...
    def scroll_bottom(self, force=False, snap=1):
        ...
        _w, l = self.chat_view.viewport().viewFullAreaSize()
        h = self.chat_view.height()
        t = l - h
        diff = l - self.fal + snap

        if force or oy >= t - diff:
            self.chat_view.viewport().viewMoveTo(ox, t)
