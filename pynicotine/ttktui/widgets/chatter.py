...
class Chatter(ttk.TTkSplitter):
    ...
    def __init__(self, chats, entity, send_message_callback, command_callback, follow=True):
        ...
        self.chat_send = ttk.TTkButton(
            parent=self.chat_bar, text=ttk.TTkString(">", ttk.TTkColor.BOLD), minWidth=5, maxWidth=5,
            addStyle={
                'default': {
                    'color': ttk.TTkColor.BOLD + ttk.TTkColor.WHITE + USER_STATUS_COLORS[UserStatus.ONLINE].invertFgBg(),
                    #"bgcolor":     ttk.TTkColor.fgbg("#222244", "#222244"),
                    #'borderColor': ttk.TTkColor.BG_BLACK  # ttk.TTkColor.fgbg("#222244", "#222244")
                },
            }
        )
        ...
        self.chat_line = ttk.TTkLineEdit(parent=self.chat_bar, hint=chat_hint)
        ...
        self.chat_emoj = ttk.TTkButton(
            parent=self.chat_bar, text='😎', minWidth=4, maxWidth=4, enabled=False
        )
        ...
        # self.chat_line.focusChanged.connect(self.on_chat_line_focus)
        # self.chat_line.currentStyleChanged.connect(self.on_chat_line_style)
        ...
        if chats.name() == "chatrooms" and entity != core.chatrooms.GLOBAL_ROOM_NAME:
            ...
            self.users_container = ttk.TTkContainer(layout=ttk.TTkVBoxLayout(), visible=False)
            self.users_header = ttk.TTkFrame(parent=self.users_container, layout=ttk.TTkHBoxLayout(), title="Users", minHeight=3, maxHeight=3)
            self.activity_view = ttk.TTkTextEdit(readOnly=True, paddingLeft=1)
            self.activity_view.document().contentsChanged.connect(self.scroll_activity)

            self.chat_paned.insertWidget(0, self.activity_view, size=2)
    ...
    @ttk.pyTTkSlot(bool)
    def on_chat_line_focus(self, _is_focused):
        pass
    ...
    def destroy(self):
        ...
        if self.activity_view is not None:
            ...
            self.layout().removeWidget(self.activity_view)  # .close()

        self.chat_container.layout().removeWidget(self.chat_view)  # .close()
        self.layout().removeWidget(self.chat_line)  # .close()
        self.layout().removeWidget(self.chat_send)  # .close()
        self.layout().removeWidget(self.chat_emoj)  # .close()
        self.layout().removeWidget(self.chat_container)  # .close()
    ...
    def _insert_line(self, line, prepend=False):
        ...
        self.chat_view.textCursor().insertText(
            ttk.TTkString("").join(ttk.TTkString(text, color) for text, color in line), moveCursor=not prepend
        )
        self._remove_old_lines(0 if prepend else self.chat_view.document().lineCount())
    ...
    def _remove_old_lines(self, num_lines):
        ...
        cursor.movePosition(moveMode=cursor.MoveAnchor, operation=cursor.Start)
        cursor.movePosition(moveMode=cursor.KeepAnchor, operation=cursor.Down, n=100,
                            textWrap=self.chat_view.textEditView()._textWrap)
        cursor.movePosition(moveMode=cursor.KeepAnchor, operation=cursor.EndOfLine)
        cursor.removeSelectedText()

        self.add_line("--- overflow ---", message_type="hilite", prepend=True)

        cursor.movePosition(moveMode=cursor.MoveAnchor, operation=cursor.End)
    ...
    def _generate_hypertext(self, text, tag=None):
        ...
        if self.parse_urls and ("://" in text or "www." in text or "mailto:" in text):
            ...
            while match:
                ...
                url = match.group()
                yield (url, ttk.TTkColor.fg(config.sections["ui"].get("urlcolor", "").upper() or "#5288CE", link=url))
                # Match remaining url
                text = text[match.end():]
                match = URL_REGEX.search(text)
    ...
    def on_send_message(self):
        ...
        if not text.isPlainText():
            text = text.toAscii().strip()
