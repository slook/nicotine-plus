...
class Dialog:
    ...
    def __init__(self, parent=None, content_box=None, buttons_box=None, default_widget=None,
                 show_callback=None, close_callback=None, title="", modal=False, width=48, height=24):
    ...
    def present(self):
        ...
        # if self.parent == ttk.TTkHelper._rootWidget:
        #     self.parent.overlay(caller=None, widget=self.window, pos=(x, y), modal=self.modal)
        # else:
    ...
    def isVisible(self):
        return (self.window is not None and self.window.isVisible())  # AndParent())
...
class MessageDialog(ttk.TTkMessageBox):
    ...
    def _on_button_selected(self, button):
        ...
        if self.callback and button not in [MessageDialog.StandardButton.Cancel, MessageDialog.StandardButton.Close]:
    ...
    def present(self):
        ...
        if self.parent == ttk.TTkHelper._rootWidget:
            self.parent.overlay(caller=self.parent, widget=self, pos=(x, y), modal=True)
        else:
            ttk.TTkHelper.overlay(None, self, x, y, modal=True)
