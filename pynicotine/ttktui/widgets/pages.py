...
class Pages(ttk.TTkTabWidget):
    ...
    def __init__(self, module, name):
        ...
        self.menu.addMenu(_("Re_open Closed Tab")).menuButtonClicked.connect(self.restore_removed_page)
        self.menu.addMenu(_("Close All Tabs…")).menuButtonClicked.connect(self.remove_all_pages)
        self.menu.addSpacer()
        self.menu.widgets = {}

        #self.setPadding(3, 0, 1, 1)
    ...
    def request_tab_changed(self, page, is_important=False, is_quiet=False):
        ...
        if has_tab_changed:
            ...
            self.update_page_menu_item(page.name())
    ...
    def update_pages_menu(self):
        ...
        for page_name in self.pages:
            self.update_page_menu_item(page_name)

    def update_page_menu_item(self, page_name):

        if page_name not in self.pages:
            if page_name in self.menu.widgets:
                self.menu.removeMenuItem(self.menu.widgets.get(page.name()))
            return

        color = self._get_page_tab_color(page_name)

        if page_name in self.unread_pages:  # ✦
            text = ttk.TTkString(f"🟋", color+ttk.TTkColor.BLINKING) + ttk.TTkString(f" {page_name} ", ttk.TTkColor.BOLD)
        else:
            text = ttk.TTkString(f"🟁", color+ttk.TTkColor.BLACK) + ttk.TTkString(f" {page_name} ")
    ...
    def update_page_tab_button(self, page_name):
        ...
        button.setText(button.text().setColor(color, posFrom=1, posTo=len(button.text())))
    ...
    def remove_page(self, page, page_args=None):
        ...
        del self.menu.widgets[page.name()]
    ...
    def on_remove_page(self, page_num):
        # unused
        page = self.widget(page_num)
        self._remove_page(page, page_args=(page.name(),))
    ...
    def set_user_status(self, button, user, status):

        icon = USER_STATUS_ICONS.get(status)
        label = icon + ttk.TTkString(f" {user} ", self._get_page_tab_color(user))
        tooltip = USER_STATUS_LABELS.get(status)

        button.setText(label)  # = page.name()
        button.setToolTip(f"{user} ({tooltip})")  # = page.name()
