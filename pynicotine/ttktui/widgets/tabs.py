...
class Tabs(ttk.TTkTabWidget):

    def __init__(self, screen, name="screen"):
        super().__init__(border=False, closable=False, name=name)

        self.screen = screen

    def destroy(self):
        self.currentChanged.disconnect(self.on_switch_tab)
        self._tabStatus.tabBar.currentChanged.clear()
        self._tabStatus.currentChanged.clear()
        self._tabStatus.statusUpdated.clear()

    def connect_signals(self):
        self.currentChanged.connect(self.on_switch_tab)
    ...
    def on_switch_tab(self, tab_index):
        ...
        self.screen.set_active_header_bar(tab)

        if isinstance(tab, Pages):
            # Show active page and focus default widget
            # tab.on_switch_page(tab.indexOf(tab.currentWidget()))
            tab.currentChanged.emit(tab.indexOf(tab.currentWidget()))
        else:
            tab.on_tab_switched()

    # Tab Highlights #

    def request_tab_changed(self, tab, is_important=False):  # , is_quiet=False):
        return self.update_tab_button(tab, tab_color="hilite" if is_important else "changed")

    def remove_tab_changed(self, tab):
        self.update_tab_button(tab)

    def update_tab_button(self, tab, tab_color="default"):

        button = self.tabButton(self.indexOf(tab))

        if button is None or button.data() == tab_color:
            # Hidden tab or tab notification already set
            return False

        button.setText(button.text().setColor(TAB_COLORS[tab_color], posFrom=0, posTo=len(button.text())))
        button.setData(tab_color)
        return True
...
class Pages(ttk.TTkTabWidget):
    ...
    def connect_signals(self):
        self.module.create_header()
        self.currentChanged.connect(self.module.on_switch_page)

    def create_pages_menu(self):

        self.menu = self.addMenu(" 0 ", position=ttk.TTkK.RIGHT)
        self.menu.widgets = {}

        self.menu.reopen = self.menu.addMenu(_("Re_open Closed Tab"))
        self.menu.reopen.menuButtonClicked.connect(self.restore_removed_page)
        self.menu.reopen.setEnabled(False)

        self.menu.closeall = self.menu.addMenu(_("Close All Tabs…"))
        self.menu.closeall.menuButtonClicked.connect(self.remove_all_pages)
        self.menu.closeall.setEnabled(False)
    ...
    def destroy(self):
        ...
        if self.menu is not None:
            self.menu.reopen.menuButtonClicked.disconnect(self.restore_removed_page)
            self.menu.closeall.menuButtonClicked.disconnect(self.remove_all_pages)
        ...
        self.module.on_remove_all_pages()
    ...
    def request_tab_changed(self, page, is_important=False, is_quiet=False):
        ...
        if not is_quiet or is_important:
            ...
            self.screen.tab_bar.request_tab_changed(self.module, is_important)
    ...
    def _remove_unread_page(self, page_name):
        ...
        if not self.unread_pages:
            self.screen.tab_bar.remove_tab_changed(self.module)
            return
        ...
        if important_page_removed and not any(is_important for is_important in self.unread_pages.values()):
            self.screen.tab_bar.remove_tab_changed(self.module)
            self.screen.tab_bar.request_tab_changed(self.module, is_important=False)
    ...
    def update_pages_count(self):
        ...
        if self.unread_pages:
            tooltip_text = ngettext("%s Unread Tab", "%s Unread Tabs", num_pages) % str(num_pages)
        else:
            tooltip_text = _("All Tabs")

        self.menu.setText(f" {num_pages} ")
        self.menu.setToolTip(tooltip_text)

    def update_pages_menu_item(self, page_name):

        if self.menu is None:
            self.create_pages_menu()
        ...
        if page_name not in self.pages and menu_item is not None:
            ...
            self.menu.closeall.setEnabled(len(self.pages) > 0)
            return
        ...
        if menu_item is None:
            ...
            self.menu.closeall.setEnabled(True)
    ...
    def remove_page(self, page, page_args=None):
        ...
        if page_args:
            ...
            self.menu.reopen.setEnabled(True)
