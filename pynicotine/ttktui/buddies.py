...
class Buddies(ttk.TTkTree):
    ...
    def add_buddy(self, user, user_data, select_row=True):
        ...
        #self.buddy_indexes[user] = self.indexOfTopLevelItem(buddy_item)
    ...
    def remove_buddy(self, user):
        ...
        del self.buddy_items[buddy_item]
...
class Buddy(ttk.TTkTreeWidgetItem):
    ...
    def on_trusted(self, is_trusted):
        core.buddies.set_buddy_trusted(self.user, is_trusted)

    @ttk.pyTTkSlot(bool)
    def on_notify(self, notify_status):
        core.buddies.set_buddy_notify(self.user, notify_status)

    @ttk.pyTTkSlot(bool)
    def on_prioritized(self, is_prioritized):
        core.buddies.set_buddy_prioritized(self.user, is_prioritized)
