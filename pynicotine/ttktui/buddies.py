...
class Buddies(ttk.TTkTree):
    ...
    def add_buddy(self, user, user_data, select_row=True):
        ...
        self.buddy_items[user] = buddy_item = BuddyItem([
            USER_STATUS_LABELS.get(status, ""),
            str(user),
            country_code,  # get_flag_icon_name(country_code),
            str(h_speed),
            str(h_files),
            ttk.TTkCheckbox(size=(3,1), text="", checked=bool(user_data.is_trusted)),  # , alignment=ttk.TTkK.CENTER_ALIGN),
            ttk.TTkCheckbox(size=(3,1), text="", checked=bool(user_data.notify_status)),  # , alignment=ttk.TTkK.CENTER_ALIGN),
            ttk.TTkCheckbox(size=(3,1), text="", checked=bool(user_data.is_prioritized)),  # , alignment=ttk.TTkK.CENTER_ALIGN),
            str(h_last_seen),
            str(user_data.note),
        ], speed_data=speed, files_data=files, last_seen_data=last_seen, selected=select_row)
    ...
    def buddy_last_seen(self, user, online):
        ...
        buddy_item.last_seen = UINT64_LIMIT
        h_last_seen = ""

        if not online:
            buddy_item.last_seen = time.time()
            h_last_seen = time.strftime("%x %X", time.localtime(buddy_item.last_seen))
    ...
    def user_stats(self, msg):
        ...
        buddy_item.speed_data = msg.avgspeed or 0
        buddy_item.files_data = msg.files or 0

        h_speed = human_speed(buddy_item.speed_data) if buddy_item.speed_data > 0 else ""
        h_files = humanize(buddy_item.files_data)
    ...
    def get_selected_username(self):
        ...
        for user_item in self.selectedItems():
            return user_item.username  # data(1)
...
class BuddyItem(ttk.TTkTreeWidgetItem):
    ...
    def __init__(self, data, speed_data=0, files_data=0, last_seen_data=0, selected=False, **kwargs):
        super().__init__(data, selected=selected, **kwargs)

        self.username = data[1]

        self.username_data = self.username.lower()
        self.speed_data = speed_data  # 3
        self.files_data = files_data  # 4
        self.last_seen_data = last_seen_data  # 8
    ...
    def sortData(self, col):
        ...
        if col == 4:
            return self.files_data or 0

        if col in [5, 6, 7]:
            return self.widget(col).isChecked()

        if col == 8:
            return self.last_seen_data
